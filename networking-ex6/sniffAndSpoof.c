...
#define theDevice   "enp0s3"                         // interface to sniff frames
...
void sniff_frame(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    printf("%s. Len %d. ICMP: type %d, code %d\n", 
     ...
     if( icmph->type == 8 && icmph->code == 0 && iph->daddr == inet_addr(dst_ip) )
     {
        ...
        snd_frm_raw(theDevice, eh->ether_dhost, frm);          // send fake reply
     }
...
int main( int argc, char *argv[] )
{
    ...
    if( (handle = pcap_open_live(theDevice, MAX_ETHER, 1, 100, errbuf)) == NULL ) {  
        fprintf(stderr, "fail in open pcap %s\n", errbuf);
        return -1;
    } 

    // Step 2: Compile filter_exp into BPF psuedo-code
    if( pcap_compile(handle, &fp, "icmp", 0, 0) == -1 )   {
        fprintf(stderr, "fail in compile %s\n", pcap_geterr(handle));
        return -1;
    }

    
    if( pcap_setfilter(handle, &fp) == -1 ) {
        fprintf(stderr, "fail in setfilter %s\n", pcap_geterr(handle));
        return -1;
    }

    
    // Step 3: Capture packets
    pcap_loop(handle, MAX_FRAMES, sniff_frame, NULL);     
